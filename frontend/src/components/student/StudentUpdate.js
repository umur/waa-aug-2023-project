...
const StudentUpdate = () => {
    ...
    const handleStudentUpdate = async (e) => {
        ...
        try {
            ...
            if(cv === initialCV){
                // cv was not changed
                updatedStudent = { firstName, lastName, username, phone, cv, isCurrentlyEmployed };
            } else {
                // cv was changed/file was selected
                const base64CV = await convertFileToBase64(cv);
                updatedStudent = { firstName, lastName, username, phone, cv: base64CV, isCurrentlyEmployed };
            }
